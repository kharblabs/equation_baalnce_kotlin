...
import com.kharblabs.balancer.equationbalancer.chemicalPlant.ConstantValues
import androidx.core.graphics.toColorInt
import com.kharblabs.balancer.equationbalancer.elementer
...
class PeriodicTableAdapter(
    ...
    lateinit var periodicGrid: List<PeriodicCell?>


    override fun getItem(position: Int): Any? = null
    private val cachedColors = gridColor.map { Color.parseColor(it) }
    private val cachedNightColors = gridColorNight.map { Color.parseColor(it) }
    override fun getItemId(position: Int): Long = 0L
    override fun getCount(): Int =   periodicGrid.size
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        val view = convertView ?: LayoutInflater.from(context).inflate(R.layout.table_element, parent, false)

        val symbol = view.findViewById<TextView>(R.id.elementSymbol)
        val name = view.findViewById<TextView>(R.id.elementName)
        val symbolNumber = view.findViewById<TextView>(R.id.elementNumber)
        val weight = view.findViewById<TextView>(R.id.elementWeight)

        val cell = periodicGrid[position]

        if (cell != null) {
            // Fill with real element data
            symbol.text = cell.symbol
            name.text = cell.name
            symbolNumber.text = cell.number.toString()
            weight.text = cell.mass.toString()

            // Optional: visible + click enabled
            view.isClickable = true
            view.visibility = View.VISIBLE

            // Apply background color per mode
            ...
            // Make the view completely blank
            symbol.text = ""
            name.text = ""
            symbolNumber.text = ""
            weight.text = ""

            view.setBackgroundColor(Color.TRANSPARENT) // or a uniform background if preferred
            view.isClickable = false
            view.visibility = View.VISIBLE // keep it visible but empty
        }
    ...
    suspend fun initData() {
        // Build heavy data on background thread
        val newGrid = withContext(Dispatchers.Default) {
            buildPeriodicGrid(consts.elements,consts.elementNames,consts.roundedMasses)
        }
